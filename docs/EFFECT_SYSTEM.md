# ATEM FX — Effect System

**Status: Implemented (M0).** The engine ships a linear `EffectChain`, a
registry, generic `ParameterSet` UI, and nine built-in effects — four from M0,
`auto_frame` from the subject tracking extension, plus `subpixel`, `fm_raster`,
`crt` and `shutter`. A DAG / graph
...
Types today: `Float`, `Int`, `Bool`. Enumerations are not a parameter type yet;
`Mirror` exposes `mode` as an int slider (0–4) until M3.
...
`App::createDefaultChain()` adds all nine; `auto_frame` starts enabled
unless `--enable` overrides. Auto Frame is first so the rest of the chain
treats the LED picture, not the wide shot. `shutter` sits after `subpixel`
so the look can smear; `crt` is last so it sits on the already-processed
picture.
...
Glitch and VHS appear in the original product vision. They are not
implemented. Do not document them as if they were. `shutter` covers the
temporal trail the vision called Trails.
...
- `drawParameters()` walks the selected node's `ParameterSet`;
- each parameter exposes Loop and generic waveform, range, cycle, phase,
  pause/restart controls with a curve preview.

An effect that needs bespoke UI code is a design smell. Adding a built-in
never requires changing this panel.
...
No shipped effect uses this yet except `TestPatternSource`, which writes
`"source.frame"`.
