...
FX-026 is the deliberately narrow early opening of M6: a managed, bounded
...
- Auto Frame sliders must visibly retarget the crop; the dead zone is for detector jitter, not operator changes. Composition is `Subject Size` + `Headroom` (vertical) + `Offset X` (horizontal looking room); there is no Offset Y because it would fight Headroom over the same axis. The COMPOSE arrow pad (and matching keyboard arrows) in the Auto Frame inspector, with Follow Subject on, nudges those same Offset X / Headroom values; arrows must move the cyan framing crop on SOURCE in the click direction (not the subject), and the pad height must be counted in the inspector height passes.
...
- Sidebar sections (SOURCE, OUTPUT, PRESETS, OVERLAYS, EFFECTS) collapse and expand on section-title click so the operator UI stays dense without docking. The left column itself can shrink to a 40 px rail of stacked titles (header chevron); clicking a title reopens the column and that section. PROGRAM modes stay in the column — two clicks while the rail is closed, never moved to the header. Effect and overlay controls open in the wide panel under the preview, which otherwise shows the stats strip. Closing that panel, folding its owning section, or opening SOURCE/OUTPUT/PRESETS returns it to the stats.
- Scene presets (FX-009 minimum) live in the PRESETS panel: factory looks plus user JSON under Application Support. Schema v2 stores the linear chain (order, enable, params, loops), the overlay stack and FX/Clean — not Freeze/Black; v1 loads with an empty overlay stack. Venue boot (`boot.json`) remembers last source, output display and Auto Frame portrait separately so a look recall cannot re-route the wall. Operation lock blocks recall/save.
- Never add Cursor references to commits or PRs (no `Co-authored-by`, no Cursor attribution); leave only the commit message.
- When the user asks to update the already-installed app, rebuild the macOS package and refresh the local `CamVJ.app` install — do not only document the steps.
...
- There is no DeckLink playback on macOS. Operator OUTPUT can still fullscreen PROGRAM onto a chosen display; loss of that wall, or a change to its raster, refresh rate, or desktop geometry, closes output and cancels pending routing — the operator must reselect; unrelated monitor churn keeps the live route (`platform/display_routing.*`, `docs/RUNTIME.md`). ATEM USB-C is one UVC webcam (typically Program), not per-input ISO live feeds; multi-camera FX is AUX→DeckLink (M1/M4). Routing the treated picture back to ATEM or Resolume is unsolved and unscoped.
- Default chain: `auto_frame` first and enabled, then passthrough / rgb_split / pixelate / fm_raster / subpixel / shutter / frame_delay / mirror / vhs / crt off, with Follow Subject on. Eleven nodes; `--enable` toggles those and creates none. Factory scene presets (Clean Frame, RGB Pulse, Pixel Stage, Shutter Trail, Echo Delay, VHS/CRT) recall looks on top of that chain without opening the M2 graph.
- FX-026 overlays are a dedicated stage after `EffectChain` and before `ProgramOutput`, never `Effect` nodes. The managed library accepts exact 1920×1080 or 1080×1920 static PNGs and 2–300-frame PNG sequences; imports are asynchronous and never take content live. The stack has at most four assets, multiple sequences may be stored disabled but only one may be active/fading, and 9:16 uses the centred strip on the fixed canvas. Steady-state decode handoff/upload/composition may not allocate, lock, log or wait; see `docs/OVERLAYS.md`.
...
- Preview is split. The right monitor is PROGRAM (what the output receives; 9:16 UV-crops the centre strip). The left monitor is a bus with two positions: SOURCE (pre-chain camera plus Tungsten subject and cyan crop overlays, and the subject picker) and FX (`chainPreview` — the chain plus managed overlays, captured before `ProgramOutput` can hold or replace it, so a look can be built and watched behind Freeze or Black before it is taken). A live subject pick forces the left monitor back to SOURCE. 9:16 output is a centred letterbox strip on the 1920×1080 canvas so Resolume can crop a static mapping.
- Only camera input feeds the tracker. Test Pattern has no CPU BGRA and reports `no frames from this input`.
- PROGRAM has four operator states in one control (FX / Clean / Freeze / Black), drawn in its own unfoldable panel at the top of the left column, above SOURCE, and available under Operation lock. `Clean` keeps `EffectRole::Framing` — it removes visual effects and managed overlays, never the shot or the 9:16 window. No PROGRAM button cuts: all four dissolve over `Dissolve::kDefaultSeconds` (0.35 s, eased, shared ramp in `src/video/program_output.h`). Two levels, because they are two kinds of change. FX/Clean *change* the picture, so `ProgramTransition` ramps `effectMix`; each visual node renders into `chain.wet` for `crossfade`, and `OverlayCompositor` applies the same amount to layer opacity; framing is never mixed. Freeze/Black *replace* the picture, so `ProgramOutput` dissolves between whole images — with live sources, not snapshots, so leaving FX for Freeze fades a moving picture into the still of the press. Effects and Clean are one `ProgramSource` there, so nothing fades twice. The latched still is pinned only while a dissolve reads it and otherwise tracks the composed pipeline; re-aiming mid-dissolve reverses, or dissolves from `program.composite`; the input-loss latch snaps because safety is not a gesture; settled modes cost no mix pass; and a missing `crossfade` shader falls back to a cut, never a dropped frame. Freeze and Black keep capture, tracking, the chain, overlay playback and the output surface running. Input loss latches Freeze and never auto-selects an input (test pattern is deliberate, never a wall fallback); a returning camera does not take itself live, and loss does not override operator Black. See `docs/RUNTIME.md` §PROGRAM safety.
